...
## SQL 语句执行流程总结
以一条典型的 `SELECT` / `INSERT` / `UPDATE` / `DELETE` 为例，数据库内部通常按如下阶段执行：

- **1) 客户端发送 SQL**
  - 应用程序通过 JDBC/ODBC、驱动或命令行工具将 SQL 文本发送到数据库连接。
  - 数据库先做连接鉴权、权限上下文装载，并为该会话准备执行环境。

- **2) 词法与语法检查（Parse）**
  - 词法分析：把 SQL 字符串拆成关键字、标识符、常量、操作符等 Token。
  - 语法分析：检查 SQL 是否符合语法规则（例如子句顺序、括号匹配、关键字使用）。
  - 语义检查：校验表、列、函数是否存在，类型是否兼容，当前用户是否有权限访问对象。
  - 若失败会直接报错（如语法错误、字段不存在、权限不足）。

- **3) 编译与优化（Optimize）**
  - 解析成功后生成逻辑执行树（关系代数表达）。
  - 查询优化器评估多种执行计划：例如全表扫描还是索引扫描、Join 顺序、Join 算法（Nested Loop / Hash Join / Merge Join）等。
  - 基于统计信息（基数、数据分布、索引选择性）估算成本，选择代价最低的物理执行计划。
  - 一些数据库会把执行计划缓存，后续同类 SQL 可复用，减少重复优化开销。

- **4) 执行阶段（Execute）**
  - 执行器按执行计划逐算子运行（Scan、Filter、Join、Sort、Aggregate）。
  - 对 DML 语句会写入数据页、生成 Undo/Redo 日志，并遵循事务隔离级别和锁机制（行锁/间隙锁/MVCC）。
  - 对查询语句会边执行边产出结果集（或先物化再返回，取决于执行计划）。

- **5) 返回结果与结束**
  - 数据库将结果或影响行数返回给客户端。
  - 在显式事务中等待 `COMMIT` / `ROLLBACK`；在自动提交模式下语句结束即提交。

### 解释执行与预编译（Prepared Statement）
- 解释执行（动态 SQL）：
  - 每次执行都要经历“解析 -> 优化 -> 执行”，灵活但频繁重复时成本更高。
- 预编译执行（Prepared Statement）：
  - 先 `PREPARE`（或驱动层预处理）生成参数化模板并缓存执行计划，再多次 `EXECUTE` 仅绑定参数。
  - 优点：减少重复解析/优化开销，提升高频 SQL 性能；同时可降低 SQL 注入风险（参数和 SQL 结构分离）。
  - 注意：并非所有场景都完全复用同一计划，复杂条件下优化器仍可能重新选择计划。

### 一句话记忆
`SQL 文本 -> 解析检查 -> 优化生成计划 -> 执行器运行 -> 返回结果（事务提交/回滚）`
